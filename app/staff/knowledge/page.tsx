    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">
            Manage academic policy documents, student handbooks, and reference guides indexed by the AI assistant.
          </p>
        </div>
      ...
      <div className="rounded-3xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xs overflow-hidden">
        {isLoading ? (
          ...
          <EmptyState
            icon={BookOpen}
            title={searchTerm || categoryFilter ? "No matching documents" : "No documents uploaded yet"}
            description={
              searchTerm || categoryFilter
                ? "Try adjusting your search query or category filter."
                : "Upload course syllabi, departmental guidelines, and handbook documents to empower AI responses."
            }
            action={
              <button
                onClick={() => setUploadModalOpen(true)}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-4 text-slate-500 dark:text-slate-400">
                        {doc.department_name || doc.source || "General"}
                      </td>
      ...
      <ConfirmDialog
        ...
        description="Deleting this document will remove its chunks and vector embeddings from AI search."
