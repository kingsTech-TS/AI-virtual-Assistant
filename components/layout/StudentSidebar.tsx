...
const NAV_ITEMS = [
  ...
  { href: "/dashboard/notifications", label: "Notifications", icon: Bell, hasBadge: true },
  ...
  return (
    <aside className="w-64 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col h-full shrink-0">
      ...
      <div className="h-16 px-6 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
        ...
        >
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Student Assistant</p>
          </div>
        </Link>
      ...
      <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto">
        ...
        {user && (user.role === "staff" || user.role === "admin" || user.role === "super_admin") && (
          <div className="pt-4 mt-4 border-t border-slate-200 dark:border-slate-800">
            ...
            >
              ...
              <span>Switch to {user.role === "staff" ? "Staff Portal" : "Admin Suite"}</span>
            </Link>
          </div>
        )}
      </nav>
